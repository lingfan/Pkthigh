...
namespace JEngine.Editor
{
    ...
    internal class ClassBindEditor : Sirenix.OdinInspector.Editor.OdinEditor
    {
        public override void OnInspectorGUI()
        {
            ...
            Setting.MakeHorizontal(50, () =>
            {
                if (GUILayout.Button(Setting.GetString(SettingString.ClassBindMatchGameObject), GUILayout.Height(30)))
                {
                    GetGameObjectByFieldName(target as ClassBind);
                }
            });
        ...
        public static async void GetGameObjectByFieldName(ClassBind instance, bool toast = true)
        {
            ...
            foreach (var data in instance.scriptsToBind) //遍历
            {
                ...
                {
                    if (field.fieldType != FieldType.GameObject && field.fieldType != FieldType.UnityComponent)
                        continue;
                    string pattern = Setting.ClassBindMatchGameObjectRegex;
                    string replacement = Setting.ClassBindMatchGameObjectReplace;
                    string result = Regex.Replace(field.fieldName, pattern, replacement);
                    Transform g = instance.gameObject.transform.Find(result);

                    if (g == null && Setting.ClassBindMatchObjectName)
                    {
                        g = FindChild(instance.transform, field.fieldName.ToLower());

                        if (g == null)
                        {
                            Log.PrintError(String.Format(Setting.GetString(SettingString.ClassBindMatchGameObjectNull),
                                instance.gameObject.name, field.fieldName));
                        }
                    }

                    if (g)
                    {
                        field.gameObject = g.gameObject;
                        affectCounts++;
                    }
            ...
            TrySave(instance, toast, String.Format(Setting.GetString(SettingString.ClassBindMatchGameObjectResult),
                    affectCounts, instance.name), Setting.GetString(SettingString.ClassBindMatchGameObjectTitle),
        ...
        public static async void DoConvert(ClassBind instance, bool toast = true)
        {
            ...
            foreach (var data in instance.scriptsToBind) //遍历
            {
                ...
                foreach (var field in members)
                {
                    ...
                    //理论上是不应该会获取private的 但理论上[SerializeField] private还是需要序列化的
                    if (Setting.ClassBindIgnorePrivate)
                    {
                        if (field is FieldInfo f)
                        {
                            if (f.IsPrivate)
                            {
                                var attr = field.GetCustomAttributes(typeof(SerializeField), false);
        ...
        private static void SetType(ClassField cf, Type type, Assembly hotCode)
        {
            ...
            else if (type.IsSubclassOf(typeof(Object)))
            {
                ...
                {
                    cf.fieldType = FieldType.HotUpdateResource;
                }
                else
                {
            ...
            else
            {
                ...
                {
                    cf.fieldType = FieldType.Number;
                }
                else if (type == typeof(string))
                {
                    cf.fieldType = FieldType.String;
                }
                ...
                else if (type.IsArray)
                {
                    ...
                    if (numType.Contains(elemType) || elemType == typeof(string) || elemType == typeof(bool))
                    {
                        cf.fieldType = FieldType.PrimitiveTypeList;
                    }
                    else
                    {
                        cf.fieldType = FieldType.UnityObjectTypeList;
                    }
                }
                ...
                {
                    cf.fieldType = FieldType.NotSupported;
                }
            }
        }
        ...
        private static Transform FindChild(Transform parentTransform, string targetName)
        {
            foreach (Transform child in parentTransform.GetComponentsInChildren<Transform>(true))
            {
                if (child.name.ToLower() == targetName)
                {
                    return child;
                }
            }
